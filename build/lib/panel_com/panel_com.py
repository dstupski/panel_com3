...
class PanelCom:
    ...
    def SetPatternID(self,patternid):
        ...
        self.send(str.encode(chr(0x02)) + str.encode(chr(0x03)) + str.encode(chr(patternid)))

    def BlinkLED( self ): self.send(str.encode(chr(0x01)) + str.encode(chr(0x50)))

    def AllOn( self ): self.send(str.encode(chr(0x01)) + str.encode(chr(0xFF)))

    def AllOff( self ): self.send(str.encode(chr(0x01)) + str.encode(chr(0x00)))

    def Greenscale( self, level ):
        if type(level) != int: raise TypeError
        if level < 0 or level > 7: raise ValueError
        self.send(str.encode(chr(0x01)) + str.encode(chr(0x40 + level)))

    def Start( self ): self.send(str.encode(chr(0x01)) + str.encode(chr(0x20)))

    def Stop( self ): self.send(str.encode(chr(0x01)) + str.encode(chr(0x30)))

    def Reset( self ): self.send(str.encode(chr(0x02)) + str.encode(chr(0x01)) + str.encode(chr(0x00)))

    def SetMode( self, modex, modey ):
        if type(modex) != int or type(modey) != int:
            raise TypeError
        if modex < 0 or modey < 0 or modex > 4 or modey > 4:
            raise ValueError
        self.send(str.encode(chr(0x03)) + str.encode(chr(0x10)) + str.encode(chr(modex)) + str.encode(chr(modey)))
    ...
    def SetGainOffset(self,gainx,offsetx,gainy,offsety):
        ...
        msg = str.encode(chr(0x05)) + str.encode(chr(0x71)) + str.encode(chr(gainx_num)) + str.encode(chr(offsetx_num)) + str.encode(chr(gainy_num)) + str.encode(chr(offsety_num))
        self.send(msg)

    def SetPositions(self,xpos,ypos):
        if type(xpos) != int or type(ypos) != int:
            raise TypeError
        if xpos < 0 or xpos > 255 or ypos < 0 or ypos > 255:
            raise ValueError
        self.send(str.encode(chr(0x05) + chr(0x70) + chr(xpos) + chr(0x00) + chr(ypos) + chr(0x00)))

    def Address( self, oldaddr, newaddr ):
        if type(oldaddr) != int or type(newaddr) != int:
            raise TypeError
        if oldaddr < 0 or newaddr < 0: raise ValueError
        self.send(str.encode(chr(0x03) + chr(0xFF) + chr(oldaddr) + chr(newaddr)))
